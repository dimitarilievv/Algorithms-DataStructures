package zad8_atleticarPolosoVreme;
...
public class Race {
    ...
    public static void competition(SLL<Athlete> prevWinners, SLL<Athlete> currYearRunners){
        SLLNode<Athlete> dvizi=prevWinners.getFirst();
        SLLNode<Athlete> temp=null;
        double max=Double.MIN_VALUE;
        while (dvizi!=null){
            if(dvizi.element.getTime()>max){
                max=dvizi.element.getTime();
                temp=dvizi;
            }
            dvizi=dvizi.succ;
        }
        dvizi=currYearRunners.getFirst();
        temp=null;
        while (dvizi!=null){
            if(dvizi.element.getTime()>max){
                temp=dvizi;
