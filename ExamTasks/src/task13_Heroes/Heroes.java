...
public class Heroes
{
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards)
    {
        ...
        int max=0;
        while(dvizi!=null){
            if(dvizi.element.power*dvizi.element.numAttacks>max){
                max=dvizi.element.power*dvizi.element.numAttacks;
                temp=dvizi;
            }
            dvizi=dvizi.succ;
        }
        int sredina=secondFriendCards.length()/2;
        dvizi=secondFriendCards.getFirst();
        for(int i=0;i<sredina;i++){
            dvizi=dvizi.succ;
        }
        secondFriendCards.insertBefore(temp.element,dvizi);
...
class Card
{
    ...
    public int znacajnost(int power,int numAttacts){
        return power*numAttacts;
    }
}
